 ...
 *
 * PORT NOTE (dsh): the original carried a ⚠ BROWSER-REACHABLE banner because
 * this module was compiled into the board-viewer's browser bundle and was the
 * only src/lib module with that constraint (`test/entrypoint-isolation.test.ts`
 * asserted it against the emitted bundle). The dsh port has no viewer bundle —
 * the viewer/panel is a separate deferred follow-up — so the constraint is
 * gone. The TYPE-ONLY-LEAF discipline this file already follows (imports
 * `WorkItem` in type position only, no node surface) is kept anyway: it is a
 * pure function and purity is the cheapest insurance if a client ever needs
 * it. Byte-identity is preserved from the first code line downward.
 *
