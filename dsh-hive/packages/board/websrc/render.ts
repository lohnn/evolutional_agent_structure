 ...
 * copied-from: web/render.ts @ 06a5c44-clean
 * deltas: import redirects to this package (all `../data/*` → ./data/*, with the
 *   node-only type faces consolidated into data/types.ts); `import type { Notice }` from the
 *   excluded notices.ts replaced by a local structural type; the inline sealed-module type
 *   `import("../../lib/board-transitions").ReattachDecision` re-pointed type-only to this
 *   package ("../src/lib/board-transitions.js"); `export` added to CSS, buildBadge,
 *   boardControlsHtml, kanbanSection + CardCtx; ONE appended tab-facing function
 *   (renderBoardSection, marked TAB ADDITION — also carries the meta line + build badge
 *   the board section needs under dsh). All other bytes untouched — see the drift test.
...
import type { Capability } from "./data/types.js"
import { DISSOLVE_THRESHOLD, SPLIT_THRESHOLD } from "./data/thresholds.js"
import type { DreamSummary, RecentArtifact } from "./data/types.js"
import type { BoardColumns } from "./data/board.js"
import type { HivemindMessage } from "./data/types.js"
import type { SessionCard, SessionMirror } from "./data/types.js"
import type { Subtask, WorkItem } from "./data/workitems.js"
import { absorbedLineage, lineageSessions, specRevisions } from "./data/lineage.js"
import { displayTitle } from "./data/placeholder-title.js"
import { recencyKey } from "./data/recency.js"
import { summarizeTodos, type TodoSubState } from "./data/todo-types.js"
import type { ActionRequired } from "./data/types.js"
import type { SessionStatusKind } from "./data/types.js"
import {
  boardTitle,
  deriveIconState,
  fullMarkSvg,
...
// notices.ts is EXCLUDED from the port (session-glue module; dead under dsh). The
// Appeal interface it carried is structural — inlined here (documented delta):
type Notice = { at: string; text: string }
 ...
 */
export function buildBadge(serverSha: string): string {
...
export interface CardCtx {
  ...
  decisions: Record<string, import("../src/lib/board-transitions.js").ReattachDecision>
 ...
 * subset).
 */
export function boardControlsHtml(state: BoardState): string {
...
/**
 * ── TAB ADDITION (WI-062 slice 3) — NOT present upstream ─────────────────────
 * The board SECTION of renderBoardBody, exposed separately for the DSH tab: the
 * ported tab host has no page chrome (the dsh shell owns it) and no data source
 * for the sessions/capabilities/dreams/messages sections (I-116 topology — those
 * opencode-era feeds are dead under dsh), so the tab renders exactly the lanes:
 * the filter-count heading, the #filter-corpus island, and kanbanSection — every
 * builder called here is the upstream one, byte-ported this file. The
 * fixture-mode line adapts the write-path wording: under dsh the write path is
 * SEALED (WI-062), not fixture-mode awar.
 */
export function renderBoardSection(state: BoardState, notices: Notice[] = []): string {
  const { dreams, messages } = state
  void dreams
  void messages
  const cardCtx: CardCtx = {
  ...
  // The meta line (workspace · generated · 15s · build badge) rides here — the
  // badge is the I-152 staleness handle (stable id="build-badge") and the tab
  // engine stamps the freshness verdict onto it after every morph. The h1 is
  // NOT ported (the dsh shell owns page chrome).
  const metaLine = `<div class="meta mono">workspace ${esc(state.workspaceRoot)} · generated ${esc(state.generatedAt)} · live refresh 15s · ${buildBadge(state.buildSha)}</div>`
  // NOT "Phase 1 · read-only" — the read-only subtitle lives in the tab shell
  // (ported visually from the slice-2 panel); this section keeps the heading +
  // live filter count + corpus island + lanes, and nothing else.
  return `${metaLine}
<h2>Board <span class="count">(${state.items.length} items · ${state.board.sessionOnly.length} session-only)</span> <span id="filter-count" class="count" title="how many work items the current filter shows / total — updated live by the filter bar"></span></h2>
${noticesHtml}
${kanbanSection(state.board, cardCtx, state.items)}
${state.writesEnabled ? "" : '<div class="meta">read-only view — the board write path stays sealed behind the hive_board_* tools (WI-062)</div>'}`
}
