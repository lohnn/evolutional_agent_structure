 ...
 * copied-from: web/icon.ts @ 06a5c44-clean
 * deltas: the three upstream `import type` lines (action-required, session-status, state) merged into,  one redirect: ./data/types.js (types-only copy, websrc/data/types.ts); all other bytes,  untouched.
 ...
 */
import type { ActionRequired, BoardState, SessionStatusKind } from "./data/types.js"
