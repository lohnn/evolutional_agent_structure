 ...
 */
import { scanArtifacts, } from "./dream-artifacts.js";
...
function searchableText(a) {
    const parts = [primaryText(a)];
    if (a.type === "insight")
        parts.push(a.previously_invisible_because);
    if (a.type === "warning")
        parts.push(a.trigger_conditions.join(" "));
    if (a.type === "songline")
        parts.push(a.encoded_principles.join(" "));
    if (a.type === "shadow") {
        parts.push(a.trigger_conditions.join(" "), a.location, a.nature);
    }
    return parts.join(" ");
}
function domainTags(a) {
    if (a.type === "insight" || a.type === "songline")
...
function lifecycleFlags(a) {
    const raw = a;
    const flags = [];
    if (raw.stale === true || raw.stale === "true")
        flags.push("stale");
...
function scoreEntries(directory, queryTokens, types) {
...
export function rankArtifacts(directory, query, opts = {}) {
    const k = opts.k ?? DEFAULT_K;
    const queryTokens = tokenise(query);
    const scored = scoreEntries(directory, queryTokens, opts.types);
    const total = scored.length;
    // Global order: score desc, stable id tiebreak
    const byScore = [...scored].sort((a, b) => b.ranked.score - a.ranked.score || a.ranked.id.localeCompare(b.ranked.id));
    ...
    for (const [floorType, floorSize] of [["shadow", SHADOW_FLOOR], ["warning", WARNING_FLOOR]]) {
        let have = 0;
        for (const s of selected.values())
            if (s.ranked.type === floorType)
                have++;
        for (const s of byScore) {
            if (have >= floorSize)
                break;
            if (s.ranked.type !== floorType || selected.has(s.ranked.id))
                continue;
