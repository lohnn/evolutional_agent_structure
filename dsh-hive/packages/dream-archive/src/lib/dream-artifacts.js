...
export function artifactsDir(directory) {
    return path.join(directory, ".opencode/dreams/artifacts");
}
export function artifactSubdir(directory, type) {
    return path.join(artifactsDir(directory), TYPE_TO_SUBDIR[type]);
}
export function artifactPath(directory, type, id) {
 ...
 */
export function nextArtifactId(directory, type) {
 ...
 */
export function parseArtifact(content, type) {
 ...
 */
export function readArtifact(directory, type, id) {
...
export function serializeInsight(a) {
    return [
...
export function serializeWarning(a) {
    return [
...
export function serializeSongline(a) {
    return [
...
export function serializeShadow(a) {
    return [
...
export function serializeArtifact(a) {
    switch (a.type) {
 ...
 */
export function writeArtifact(directory, artifact) {
    const type = artifact.type;
    const id = artifact[`${type}_id`];
    const filePath = artifactPath(directory, type, id);
 ...
 */
export function scanArtifacts(directory, types) {
    const targetTypes = types ?? ["insight", "warning", "songline", "shadow"];
    const entries = [];
...
function artifactConfidence(a) {
    if (a.type === "insight")
        return a.confidence;
    if (a.type === "warning")
        return a.confidence;
    if (a.type === "songline")
        return a.transfer_rating;
    return undefined; // shadows have no confidence field
}
/** Get domain_tags for an artifact, if any. */
function artifactDomainTags(a) {
    if (a.type === "insight")
        return a.domain_tags;
    if (a.type === "songline")
        return a.domain_tags;
    return [];
}
const FULL_CONTENT_THRESHOLD = 20;
export function queryArtifacts(directory, filter) {
...
export function pathForId(directory, id) {
    const type = idToType(id);
    if (!type)
        return null;
 ...
 */
export function listArtifacts(directory, opts = {}) {
    const targetTypes = opts.types ?? ["insight", "warning", "songline", "shadow"];
    const entries = [];
    for (const type of targetTypes) {
        const subdir = artifactSubdir(directory, type);
        let files;
        ...
        for (const file of files) {
            ...
            // source_dream filter
            if (opts.source_dream && source_dream !== opts.source_dream)
                continue;
 ...
 */
export function appendFieldsToArtifact(filePath, fields) {
...
function dreamOrdinal(sourceDream) {
 ...
 */
export function detectDuplicateCandidates(directory, minScore = 0.35, maxScore = 1.0) {
