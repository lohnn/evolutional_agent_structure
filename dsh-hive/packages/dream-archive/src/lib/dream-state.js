...
import fs from "fs";
// ── Paths ─────────────────────────────────────────────────────────────────────
export function dreamsBase(directory) {
    return path.join(directory, ".opencode/dreams");
}
export function activeDreamPath(directory, id) {
    return path.join(dreamsBase(directory), "active", `${id}.yaml`);
}
export function historyDreamPath(directory, id) {
 ...
 */
export function nextDreamId(directory) {
 ...
 */
export function listActiveDreams(directory) {
 ...
 */
export function recentPreCompactionDreams(directory, limit = 5) {
 ...
 */
export function parseDreamState(content) {
    ...
    while (i < lines.length) {
        ...
        if (rest === "") {
            ...
            if (j < lines.length && lines[j].match(/^\s+-\s/)) {
                ...
                while (i < lines.length) {
                    const bLine = lines[i];
                    if (bLine.trim() === "" || bLine.trim().startsWith("#")) {
                        i++;
                        continue;
                    }
                    if (!bLine.match(/^\s+-\s/))
...
export function readDreamState(filePath) {
...
export function serializeDreamState(d) {
...
export function beginDream(directory, state) {
 ...
 */
function rewriteCompletionScalars(content, exitTime) {
    const lines = content.split("\n");
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.startsWith("exit_time: "))
            lines[i] = `exit_time: ${exitTime}`;
 ...
 * untouched.
 */
export function completeDream(directory, exitTime, artifactIds) {
    ...
    if (!activeFile)
        throw new Error("NO_ACTIVE_DREAM");
