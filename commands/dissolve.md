...
The capability's pattern is archived in the void (`.opencode/agents/dissolved/`). If similar need arises, HIVE can resurrect and mutate rather than spawn from nothing.
...
Dissolution will:
- Archive capability to .opencode/agents/dissolved/
...
# Add dissolution metadata
# Move to dissolved/
mv .opencode/agents/capabilities/[name].md .opencode/agents/dissolved/[name].md
```
...
Archived: .opencode/agents/dissolved/postgres-ops.md
...
After the capability file is moved, immediately trigger a reload so the dissolved agent is no longer accessible in the current session:
...
```
echo "$(date -Iseconds) DISSOLVE: postgres-ops (energy: 12, reason: inactivity)" >> .opencode/evolution.log
```
